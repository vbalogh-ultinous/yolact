...
while read f; do
    ...
    mkdir -p "$OUTDIR"
    echo "Grayscale ON!"
    python3 custom_eval.py --grayscale --trained_model $MODEL --config $CONFIG --score_threshold $SCORE --top_k 100 --video_multiframe 4 --video $INDIR/$f:$OUTDIR/out_${SCORE}_$f --coco_class person --display_masks 0 --csv $OUTDIR/csv/$f.csv
