...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    ...
    if num_dets_to_consider == 0:
        ...
        return (img_gpu * 255).byte().cpu().numpy(), json_data
    ...
    if args.display_text or args.display_bboxes:
        ...
        for j in reversed(range(num_dets_to_consider)):
            ...
            json_det['class'] = cfg.dataset.class_names[classes[j]]
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    img_numpy, json_data = prep_display(preds, frame, None, None, undo_transform=False)
    ...
    if image_save_path is None:
        ...
        if args.csv is not None and 'detections' in json_data.keys(): # yolact has detections on current image
